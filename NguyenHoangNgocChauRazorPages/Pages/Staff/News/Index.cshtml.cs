...
public class IndexModel(INewsService newsService, ICategoryService categoryService, ITagService tagService) : PageModel
{
    ...
    public async Task<IActionResult> OnPostSaveAsync(){if(!IsStaff())return RedirectToPage("/Auth/Login");if(Input.CategoryID<=0)ModelState.AddModelError("Input.CategoryID","Category is required.");if(!ModelState.IsValid){ShowModal=true;await LoadAsync();return Page();}var staffId=HttpContext.Session.GetUser()!.AccountID;var action=string.IsNullOrWhiteSpace(Input.NewsArticleID)?"created":"updated";await newsService.SaveAsync(Input,staffId);TempData["Success"]=action=="created"?"News created.":"News updated.";return RedirectToPage(new{SearchTerm});}
    public async Task<IActionResult> OnPostDeleteAsync(string id){if(!IsStaff())return RedirectToPage("/Auth/Login");if(await newsService.DeleteAsync(id)){TempData["Success"]="News deleted.";}else TempData["Error"]="News article was not found.";return RedirectToPage(new{SearchTerm});}
